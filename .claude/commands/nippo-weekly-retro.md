...
4. **前週の振り返りファイルの読み込み**
   - `~/work/nippo/{START の 7 日前}-week.md` を Read する。存在しなければ Try 追跡（Phase 3 / Phase 4 の該当部分）はスキップする。
...
- 提示する内容:
  ...
  - **前週の Try の追跡結果**（前週ファイルがある場合）。前週の Try 各項目について、今週のデータ（日報・PR・Jira）から達成状況の仮判定と根拠を示す。
- 前週 Try のうちデータから判定できない項目は `AskUserQuestion` で本人に達成状況を確認する（❌ 未着手 / 🔺 部分的に実施 / ✅ 実施 の粒度）。
...
（前週ファイルがある場合のみ。前週の Try 各項目を ❌ 未着手 / 🔺 部分的に実施 / ✅ 実施 で判定し、根拠を 1〜2 文添える）

- **{前週の Try 項目}**: ❌/🔺/✅ {判定理由・今週どうだったか}
...
- **前週の Try で未達（❌ / 🔺）の項目は、今週の Try に「（先週から持ち越し）」として再掲する**。書きっぱなしにせず KPT のループを閉じる。
